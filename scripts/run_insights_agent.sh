#!/usr/bin/env bash
# Start the SwiftCare Insights Agent via ADK web UI.
set -euo pipefail
...
export AGENT_TYPE="${AGENT_TYPE:-insights}"
export AGENT_NAME="${AGENT_NAME:-swiftcare_insights_agent}"
export LOG_QUERIES_TO_BQ="${LOG_QUERIES_TO_BQ:-TRUE}"
export DEFAULT_AT_RISK_LIMIT="${DEFAULT_AT_RISK_LIMIT:-50}"
export MAX_ALERTS_PER_TURN="${MAX_ALERTS_PER_TURN:-10}"
export DEDUPE_OPEN_ALERTS="${DEDUPE_OPEN_ALERTS:-TRUE}"
...
echo "Agent:    agents.insights"
echo "Starting adk web ..."

exec "$ADK_BIN" web --port "${ADK_PORT:-8002}" agents
