...
# SwiftCare's live demo service is in asia-south1. Keep this default aligned
# with cloudbuild.yaml so an unset local config cannot create a second service.
...
# Cloud Build normally separates substitutions with commas. CORS legitimately
# contains commas, so use the documented alternate `|` delimiter instead.
