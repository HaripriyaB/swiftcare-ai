"""Parameterized BigQuery client for Retrieval Agent tools.
...
def get_project_id() -> str:
    return os.getenv("GCP_PROJECT_ID") or os.getenv("GOOGLE_CLOUD_PROJECT") or "swiftcare-patchamomma"
...
def _assert_allowlisted(sql: str) -> None:
    """Reject SQL that references tables outside the Chunk 2 allowlist."""
    lowered = sql.lower()
    # Skip pure DML helper statements that only touch ops via logging module
    if "swiftcare_ops" in lowered and (
        "swiftcare_fhir_views" not in lowered
        and "swiftcare_agent_cache" not in lowered
        and "swiftcare_fhir_raw" not in lowered
        and "swiftcare_fhir_analytics" not in lowered
    ):
        return
    ...
    has_allowed = any(suffix.lstrip(".") in lowered for suffix in (
        "swiftcare_fhir_views",
        "swiftcare_agent_cache.mv_patient_latest_vitals",
        "swiftcare_ops",
    ))
    if not has_allowed:
