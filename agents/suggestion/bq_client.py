"""Parameterized BigQuery client for Suggestion Agent tools.
...
def _assert_allowlisted(sql: str) -> None:
    """Reject SQL that references tables outside the Chunk 3 allowlist."""
    lowered = sql.lower()
    ...
    if "swiftcare_agent_cache" in lowered:
        raise ValueError(
            "SQL references Insights/Retrieval cache — not allowed for Suggestion Agent"
        )

    if "swiftcare_fhir_views" not in lowered and "swiftcare_ops" not in lowered:
