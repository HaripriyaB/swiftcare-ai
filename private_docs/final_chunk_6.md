...
4. Default list returns active only unless `?status=all`
...
WHERE patient_id = @patient_id
  AND is_active  -- default; allow ?status=all if useful
```
...
1. Resolve `user_id` from auth; load/create session; if body `patient_id` set, upsert `sessions.active_patient_id`.
...
gcloud run deploy "$SERVICE" \
  ...
  --region "$REGION" \
  --platform managed \
  --allow-unauthenticated \  # or authenticated + IAP for stricter demos
