...
def clinical_findings_for(patient_id: str) -> list[dict[str, Any]]:
    """Fixture counterpart of FHIR chart findings for local UI evaluation."""
    findings = []
    for row in conditions_for(patient_id)[:6]:
        findings.append(
            {
                "finding_id": f"condition:{row.get('condition_id')}",
                "patient_id": patient_id,
                "display_name": row.get("display_name", "Condition"),
                "source_kind": "FHIR Condition",
                "recorded_date": row.get("onset_date"),
                "value_text": None,
...
def at_risk(
    *, risk_flag: str | None, risk_level: str | None, limit: int, offset: int = 0
) -> list[dict[str, Any]]:
